...
class CSRFScanner:
    def __init__(self, target: str, threads: int = 40):
        ...
        self.sensitive_endpoints = [
            ...
            # Admin actions
            ("/api/admin/user/delete", "DELETE"),
            ("/api/admin/user/update", "PUT"),
    ...
    async def test_endpoint(self, endpoint: str, method: str) -> Optional[Dict]:
        ...
        # Extract form parameters first
        inputs, form_info = await self.extract_form_parameters(endpoint)
        
        async with HTTPClient(self.target, timeout=5, retries=1) as client:
            # Prepare request without CSRF token
            ...
            if response:
                ...
                if response.status == 200 or response.status == 201:
                    ...
                    if form_info.get("has_csrf"):
                        # Endpoint has CSRF protection in form, but API accepts without token
                        ...
                        return {
                            ...
                            "details": f"Endpoint accepts request without CSRF token despite having CSRF protection in forms"
                        }
                    else:
                ...
                for indicator in self.error_indicators:
                    if indicator in body_lower and ('missing' in body_lower or 'invalid' in body_lower):
                        # CSRF protection exists and working
                        return None
    ...
    async def scan(self, custom_endpoints: List[Tuple[str, str]] = None) -> Dict:
        ...
        for endpoint, method in endpoints:
            ...
            # Layer 1: Missing CSRF protection
            result = await self.test_endpoint(endpoint, method)
            if result:
                self.findings.append(result)
                icon = "🔴" if result.get('severity') == 'high' else "🟠"
                logger.finding(f"{icon} CSRF Vulnerability on {endpoint}", result.get('details', ''))
                continue
            
            # Layer 2: Token reuse vulnerability
            result = await self.test_token_reuse(endpoint, method)
            if result:
                self.findings.append(result)
                logger.finding(f"🟠 Token Reuse on {endpoint}", result.get('details', ''))
    ...
    @staticmethod
    async def run(target: str, custom_endpoints: List[Tuple[str, str]] = None) -> Dict:
        """Run CSRF scanner"""
